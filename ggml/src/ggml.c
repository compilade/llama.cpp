...
struct ggml_tensor * ggml_ssm_scan(
        ...
        struct ggml_tensor  * C,
        struct ggml_tensor  * D) {
    const bool vector_A = ggml_is_vector(A);
    ...
    if (ggml_is_vector(A)) {
        ...
        GGML_ASSERT(A->ne[0] == d_state || A->ne[0] == 1);
        GGML_ASSERT(A->ne[1] == head_dim);
        GGML_ASSERT(B->ne[0] == d_state);
        GGML_ASSERT(B->ne[2] == n_seq_tokens);
        GGML_ASSERT(B->ne[3] == n_seqs);
        GGML_ASSERT(D->ne[0] == n_head);
        // GGML_ASSERT(D->ne[0] == head_dim);
        // GGML_ASSERT(D->ne[1] == n_head);
    } else {
        GGML_ASSERT(ggml_is_matrix(A));
    }
...
static void ggml_compute_forward_ssm_scan_f32(
    ...
    const struct ggml_tensor * src2 = dst->src[2]; // dt {dim, n_head, n_seq_tokens, n_seqs}
    const struct ggml_tensor * src3 = dst->src[3]; // A  {d_state, dim} or {n_head}
    ...
    const struct ggml_tensor * src6 = dst->src[6]; // D  {d_inner} or {n_head}
    ...
    const int64_t n_h = src1->ne[1]; // n_head
    const int64_t n_g = src4->ne[1];
    const int64_t n_t = src1->ne[2]; // number of tokens per sequence
    const int64_t n_s = src0->ne[3]; // number of sequences in the batch
    ...
    for (int i3 = 0; i3 < n_s; ++i3) {
        for (int i2 = 0; i2 < n_t; ++i2) {
            // TODO: parallelize over the heads instead of their size?
            //       But Mamba-1 only has 1 head...
            for (int h = 0; h < n_h; ++h) {
                const float * s0 = (const float *) ((const char *) src0->data + ir0*(src0->nb[1]) + h*(src0->nb[2]) + i3*(src0->nb[3])); // {d_state, dim, n_h, n_s}
                const float * x  = (const float *) ((const char *) src1->data + ir0*(src1->nb[0]) + h*(src1->nb[1]) + i2*(src1->nb[2]) + i3*(src1->nb[3])); // {dim, n_h, n_t, n_s}
                const float * dt = (const float *) ((const char *) src2->data + ir0*(src2->nb[0]) + h*(src2->nb[1]) + i2*(src2->nb[2]) + i3*(src2->nb[3])); // {dim, n_h, n_t, n_s}
                const float * A  = (const float *) ((const char *) src3->data + ir0*(src3->nb[1])); // {d_state, dim}
                // TODO: avoid modulo here
                const float * B  = (const float *) ((const char *) src4->data + (h%n_g)*(src4->nb[1]) + i2*(src4->nb[2]) + i3*(src4->nb[3])); // {d_state, n_g, n_t, n_s}
                const float * C  = (const float *) ((const char *) src5->data + (h%n_g)*(src5->nb[1]) + i2*(src5->nb[2]) + i3*(src5->nb[3])); // {d_state, n_g, n_t, n_s}
                const float * D  = (const float *) ((const char *) src6->data + ir0*(src6->nb[0]) + h*(src6->nb[1])); // {dim, n_h}
                float * y = (float *) ((char *) dst->data + ir0*(src1->nb[0]) + i2*(src1->nb[2]) + i3*(src1->nb[3])); // {dim, n_h, n_t, n_s}
                float * s = (float *) ((char *) dst->data + ir0*(src0->nb[1]) + h*(src0->nb[2]) + i3*(src0->nb[3]) + (src1->nb[3]*src1->ne[3])); // {d_state, dim, n_h, n_s}

                // use the output as the source for the next token-wise iterations
                if (i2 > 0) { s0 = s; }

                if (src3->ne[0] == 1) {
                    // Mamba-2 has a scalar decay factor per state row; dA can be outside the state-wise loop

                    // dim
                    for (int i1 = 0; i1 < ir; ++i1) {
                        // ref: https://github.com/state-spaces/mamba/blob/62db608da60f6fc790b8ed9f4b3225e95ca15fde/mamba_ssm/ops/triton/softplus.py#L16
                        float dt_soft_plus = dt[i1 + h*nr] <= 20.0f ? log1pf(expf(dt[i1])) : dt[i1];
                        float dA = expf(dt_soft_plus * A[i1]);
