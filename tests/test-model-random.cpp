    ...
    static void insert_from_arch(std::vector<model_variant> & variants, llm_arch arch) {
        ...
        switch (arch) {
            ...
            case LLM_ARCH_GRANITE_MOE_HYBRID:
                {
                    variants.push_back(model_variant(arch, "Granite-MoE-Hybrid"));
                    model_variant & cur = variants.back();
                    ...
                    const uint32_t n_group = 2;
                    const uint32_t head_dim = 64;
                    const uint32_t n_ssm_head = d_inner / head_dim;
                    ...
                    const uint32_t n_embd_k_gqa = n_embd_head_k * n_head_kv;
                    const uint32_t n_embd_v_gqa = n_embd_k_gqa;
                    const uint32_t n_ff_shexp = 48;
                    const uint32_t n_expert = 6;
                    const std::vector<int32_t> attn_layer_ids = { 2 };
                    ...
                    cur.add_kv(LLM_KV_BLOCK_COUNT, n_layer);
                    cur.add_kv(LLM_KV_CONTEXT_LENGTH, (uint32_t) 131072);
                    cur.add_kv(LLM_KV_EMBEDDING_LENGTH, n_embd);
                    cur.add_kv(LLM_KV_FEED_FORWARD_LENGTH, n_ff);
                    cur.add_kv(LLM_KV_ATTENTION_HEAD_COUNT, n_head);
                    cur.add_kv(LLM_KV_ATTENTION_HEAD_COUNT_KV, n_head_kv);
                    cur.add_kv(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, 1e-5f);
                    cur.add_kv(LLM_KV_ROPE_DIMENSION_COUNT, n_embd / n_head);
                    cur.add_kv(LLM_KV_ATTENTION_SCALE, 0.0078125f);
                    cur.add_kv(LLM_KV_EMBEDDING_SCALE, 12.0f);
                    cur.add_kv(LLM_KV_RESIDUAL_SCALE, 0.22f);
                    cur.add_kv(LLM_KV_LOGIT_SCALE, 6.0f);
                    cur.add_kv(LLM_KV_EXPERT_COUNT, n_expert);
                    cur.add_kv(LLM_KV_EXPERT_USED_COUNT, (uint32_t) 2);
                    cur.add_kv(LLM_KV_EXPERT_SHARED_FEED_FORWARD_LENGTH, n_ff_shexp);
                    cur.add_kv(LLM_KV_SSM_CONV_KERNEL, d_conv);
                    cur.add_kv(LLM_KV_SSM_INNER_SIZE, d_inner);
                    cur.add_kv(LLM_KV_SSM_STATE_SIZE, d_state);
                    cur.add_kv(LLM_KV_SSM_TIME_STEP_RANK, n_ssm_head);
                    cur.add_kv(LLM_KV_SSM_GROUP_COUNT, n_group);
                    cur.add_kv(LLM_KV_ATTENTION_LAYER_INDICES, attn_layer_ids);
                    ...
                    for (uint32_t i = 0; i < n_layer; ++i) {
                        ...
                        if (std::find(attn_layer_ids.begin(), attn_layer_ids.end(), i) != attn_layer_ids.end()) {
                            const uint32_t n_head_i = n_head;
                            const uint32_t n_embd_k_gqa_i = n_embd_k_gqa;
                            const uint32_t n_embd_v_gqa_i = n_embd_v_gqa;

                            cur.add_tensor(tn(LLM_TENSOR_ATTN_Q,   "weight", i), {n_embd, n_embd_head_k * n_head_i});
                            cur.add_tensor(tn(LLM_TENSOR_ATTN_K,   "weight", i), {n_embd, n_embd_k_gqa_i});
                            cur.add_tensor(tn(LLM_TENSOR_ATTN_V,   "weight", i), {n_embd, n_embd_v_gqa_i});
                            cur.add_tensor(tn(LLM_TENSOR_ATTN_OUT, "weight", i), {n_embd_head_k * n_head_i, n_embd});
                            ...
                            cur.add_tensor(tn(LLM_TENSOR_SSM_DT, "bias", i), {n_ssm_head}, init_bias);

                            // no "weight" suffix for these
                            cur.add_tensor(tn(LLM_TENSOR_SSM_A, i), {1, n_ssm_head}, init_A);
                            cur.add_tensor(tn(LLM_TENSOR_SSM_D, i), {1, n_ssm_head}, []() { return 1.0f; });
                        ...
                        if (n_expert > 0) {
                            ...
                            cur.add_tensor(tn(LLM_TENSOR_FFN_NORM, "weight", i), {n_embd});
                            cur.add_tensor(tn(LLM_TENSOR_FFN_GATE_INP,  "weight", i), {n_embd, n_expert});
                            cur.add_tensor(tn(LLM_TENSOR_FFN_GATE_EXPS, "weight", i), {n_embd,   n_ff, n_expert});
                            cur.add_tensor(tn(LLM_TENSOR_FFN_DOWN_EXPS, "weight", i), {  n_ff, n_embd, n_expert});
                            cur.add_tensor(tn(LLM_TENSOR_FFN_UP_EXPS,   "weight", i), {n_embd,   n_ff, n_expert});
